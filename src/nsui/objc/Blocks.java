 ...
 *
 * <p>This lets any C/ObjC API that takes a block (e.g. {@code dispatch_async})
 * invoke Java logic. Only the "global block" flavor is used:
 * <ul>
 *   <li>Blocks whose {@code isa} is {@code _NSConcreteGlobalBlock} are never copied —
 *       {@code Block_copy} returns {@code self} — so no copy/dispose functions are
 *       needed and the block safely outlives the frame that created it.</li>
 *   <li>Avoids the {@code _NSConcreteStackBlock} (stack) flavor, which would require
 *       copy/dispose trampolines and careful liveness handling.</li>
 * </ul>
 *
 * <p>Block struct layout (64-bit, this toolkit supports x86_64 and arm64):
 * <pre>
 *   Block_layout {
 ...
 * </pre>
 *
 * <p>{@code _NSConcreteGlobalBlock} and {@code _NSConcreteStackBlock} are exported
 * DATA symbols of {@code /usr/lib/libSystem.B.dylib}; the isa value for a block of a
 * given class is the <em>address of that symbol itself</em> (the symbol is the block
...
public final class Blocks {
     ...
     *                 pointer (blockSelf) as its first argument. For {@code void(^)(void)}
     *                 the target type is {@code (MemorySegment blockSelf) -> void}.
     * @param fd       the FULL upcall descriptor INCLUDING the leading block-pointer
     *                 parameter. e.g. {@code FunctionDescriptor.ofVoid(PTR)} for a
     *                 no-arg void block.
