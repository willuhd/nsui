...
///
/// The msgSend surface lives in `Sig` (the signature-keyed vocabulary);
...
///
/// Three consumers:
/// - at RUNTIME, `ObjC.init()` and `WindowCheck.init()` build the
///   downcall handles from these descriptors;
/// - at BUILD time, `NsuiFeature` registers the exact same descriptors
///   with the native-image builder via the `RUNTIME`/`CORE`
/// lists — no tracing agent required, and the registered set can never
/// drift from the one used at run time.
